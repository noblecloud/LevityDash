...
Milestones: ~~wire codec~~ → ~~`RemoteContainer` + in-process loopback mode~~ (both done — verified live against real plugin data with `[Backend] mode = loopback`) → standalone backend process → frontend WS client (reconnect/attach) → timeseries + plugin control plane.
