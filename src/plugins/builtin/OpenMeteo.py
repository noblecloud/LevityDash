...
from src.plugins.translator import TranslatorSpecialKeys as tsk
...
translator = {
	...
	'timestamp':                                {'type': 'datetime', 'sourceUnit': 'ISO8601', 'format': {'hourly': '%Y-%m-%dT%H:%M', 'daily': '%Y-%m-%d'}, 'title': 'Time', 'description': 'Time', 'sourceKey': 'time', tsk.meta: '@timestamp'},
...
class OpenMeteoURLs(URLs, base='api.open-meteo.com/v1'):
	params = {'latitude': config.lat, 'longitude': config.lon, 'timezone': config.tz}
