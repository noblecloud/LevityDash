...
from PySide2.QtGui import QColor, QDrag, QFocusEvent, QFont, QPainter, QPixmap, QRegion, QTextBlockFormat, QTransform
from WeatherUnits.base import Measurement
from WeatherUnits.time.time import Second

from src.Modules.DateTime import baseClock
from src.catagories import CategoryItem
from src.fonts import weatherGlyph
from src.Modules.Displays.Text import Text, TextHelper
from src import logging
from functools import cached_property

from json import dumps
from PySide2.QtCore import QByteArray, QEvent, QMimeData, QRectF, Qt, QThread, QTimer, Signal, Slot
from PySide2.QtWidgets import QGraphicsBlurEffect, QGraphicsItem, QGraphicsScene, QGraphicsSceneHoverEvent, QGraphicsSceneMouseEvent, QGraphicsSceneWheelEvent, QGraphicsTextItem, QStyleOptionGraphicsItem, QWidget

from src.plugins.dispatcher import ValueDirectory, MultiSourceContainer, MonitoredKey, PlaceholderSignal
from src.Grid import Grid
from src.Modules.Handles.Resize import MeasurementUnitSplitter, ResizeHandle, Splitter, TitleValueSplitter
from src.Modules import hook, Panel
from src.Modules.Menus import RealtimeContextMenu
from src.utils import addCrosshair, AlignmentFlag, clamp, connectSignal, disconnectSignal, DisplayType, JsonEncoder, Margins, mouseHoldTimer, Now, Subscription, TitleCamelCase, DisplayPosition
from src.Modules.Label import EditableLabel, NonInteractiveLabel as Label, TitleLabel
...
class Realtime(Panel):
	...
	def __updateTimeOffsetLabel(self):
		if self.valueLink.forecastOnly:
			...
			self.timeOffsetLabel.setEnabled(False)
		elif abs(offset := Now() - self.value.timestamp) > timedelta(minutes=15):
			self.timeOffsetLabel.setEnabled(True)
		else:
			self.timeOffsetLabel.setEnabled(False)
...
class DisplayLabel(Panel):
	...
	def state(self):
		...
		if len(self.valueTextBox.textBox.enabledFilters):
			valueState['filters'] = list(self.valueTextBox.textBox.enabledFilters)
