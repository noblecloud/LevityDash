...
from src.EasyPath import EasyPath
from src.utils.geometry import AlignmentFlag, Position
...
class BaseContextMenu(QMenu):
	...
	def addPanel(self):
		from src.ui.frontends.PySide.Modules import Panel
		position = Position(self.position)
		item = Panel(self.parent, position=position)
		item.setFocus(Qt.FocusReason.MouseFocusReason)

	def addLabel(self):
		from src.ui.frontends.PySide.Modules.Label import EditableLabel
...
class InsertMenu(QMenu):
	...
	def insertClock(self):
		from src.ui.frontends.PySide.Modules import Clock
		clock = Clock(self.parent.parent, position=self.parent.position)

	def insertMoon(self):
		from src.ui.frontends.PySide.Modules import Moon
		moon = Moon(self.parent.parent, position=self.parent.position)

	def insertGraph(self):
		from src.ui.frontends.PySide.Modules.Displays.Graph import GraphPanel
		graph = GraphPanel(self.parent.parent, position=self.parent.position)

	def insertSplitPanel(self):
		from src.ui.frontends.PySide.Modules.Containers import SplitPanel
...
class TemplateMenu(QMenu):
	...
	def __init__(self, parent, directoryPath: Union[str, Path], *args, **kwargs):
		from src.ui.frontends.PySide.Modules.Panel import PanelFromFile
		self.parent = parent
